...
parser.add_argument('--dataset', metavar='DATASET', default='cifar10',
                        choices=['cifar10','svhn','mnist','cinic','tiny','stl10','covid2','covid3','brain4','fashion'],
...
parser.add_argument('--arch', default='cnn13', type=str, help='either of cnn13,c, WRN28_2 , cifar_shakeshake26')
...
if args.dataset == 'cifar10':
    ...
    #from dataloadercinic import dataloader
    len_data = args.num_labeled
...
if args.dataset == 'fashion':
    data_source_dir = args.data_dir
    trainloader, validloader, unlabelledloader, testloader, num_classes = load_data_subset(1, args.batch_size, args.n_cpus ,'fashion', data_source_dir, labels_per_class = args.num_labeled, valid_labels_per_class = args.num_valid_samples)
...
def getNetwork(args, num_classes, ema= False):
    ...
    if args.arch in ['cnn13','WRN28_2']:
        ...
        #net=cifar10(n_channel=128)        
        net = classifier(input_dim=1, output_dim=num_classes, input_size=28)         
...
def train_sl(trainloader, model, optimizer, epoch, filep):
    ...
    for i, (input, target)in enumerate(trainloader):
        ...
        prec1, prec5 = accuracy(class_logit.data, target_var.data, topk=(1, 4))
...
def train(trainloader,unlabelledloader, model, ema_model, optimizer, epoch, filep):
    ...
    for o,(u, _) in enumerate(unlabelledloader):
        ...
        C_unlabeled_loss = nll_loss_neg2(C_fake_wei, C_fake_pred)
        ...
        C_fake_lossmix = nll_loss_neg(outputsm, l3,wcw2)
        ...
        vat_loss = VATLoss(xi=10, eps=1, ip=1)
        #lds = vat_loss(model, u_var)

        ldsf ,dvatimageo,dvatimagea,dvatimageoa= vat_loss(model, G_.detach())
 
        #lossv =  1* lds
        lentropyf=entropy_loss(C_fake_pred)
        #output_u = model(u_var)        
        #lentropy=entropy_loss(output_u)
        ...
        #ldsu = vat_loss(model, mixedup_x)
 
        #lentropyu=entropy_loss(output_mixed_u)


        #generated_weight(epoch) * C_fake_loss   
        #+lentropyu+ldsu     
        #lentropyf+ldsf+
        loss = C_unlabeled_loss+0.1* C_fake_loss+lentropyf+ldsf+class_loss#+mixup_consistency_loss#+0.1*C_fake_lossmix+C_labeled_loss +C_unlabeled_loss+generated_weight(epoch)*C_u_lossmix
        meters.update('loss', loss.item())
        ...
        ema_prec1, ema_prec5 = accuracy(ema_logit_labeled.data, target_var.data, topk=(1, 4))
        ...
        D_real = D(input_var)
        D_labeled_loss = BCEloss(D_real, torch.ones_like(D_real))

        D_realu = D(u_var)
        D_unlabeled_loss = BCEloss(D_realu, torch.ones_like(D_realu))
        ...
        G_loss=G_loss_D+ 0.1 * G_loss_C#+0.1*G_C_fake_lossix
    ...
    with torch.no_grad():             
                vutils.save_image(dvatimageo,
                'experiments/s/dvat/ointer_samples%s.png' % (epoch + 1),
                normalize=True)    
                vutils.save_image(dvatimageo,
                'experiments/s/dvat/ointer_samples%s.pdf' % (epoch + 1),
                normalize=True)    
                vutils.save_image(dvatimagea,
                'experiments/s/dvat/ainter_samples%s.png' % (epoch + 1),
                normalize=True)    
                vutils.save_image(dvatimagea,
                'experiments/s/dvat/ainter_samples%s.pdf' % (epoch + 1),
                normalize=True)
                vutils.save_image(dvatimageoa,
                'experiments/s/dvat/oainter_samples%s.png' % (epoch + 1),
                normalize=True)    
                vutils.save_image(dvatimageoa,
                'experiments/s/dvat/oainter_samples%s.pdf' % (epoch + 1),
                normalize=True)                      
...
def validate(eval_loader, model, global_step, epoch, filep, ema = False, testing = False):
    ...
    for i, (input, target) in enumerate(eval_loader):
        ...
        prec1, prec5 = accuracy(output1.data, target_var.data, topk=(1, 4))
